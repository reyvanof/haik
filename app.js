                ...
                <td>
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                        <button class="btn btn-sm btn-blue" type="button"
                            onclick="window.openEditBrangkasItem('${itemName.replace(/\/g, '\\').replace(/'/g, "\\'")}')">
                            Edit
                        </button>
                        <button class="btn btn-sm btn-red" type="button"
                            onclick="window.deleteBrangkasItem('${itemName.replace(/\/g, '\\').replace(/'/g, "\\'")}')">
                            Hapus
                        </button>
                    </div>
                </td>
...
window.renderRecentStockActivity = function() {
    const tbody = document.getElementById('tbody-stock-activity');
    if (!tbody) return;

    const records = [...(window.stockFlowData || [])]
        .sort((a, b) => getStockFlowDate(b) - getStockFlowDate(a))
        .slice(0, 8);

    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="dashboard-empty">Belum ada aktivitas barang.</td></tr>';
        return;
    }

    tbody.innerHTML = records.map(record => {
        const badgeClass = record.direction === 'MASUK'
            ? 'flow-in'
            : (record.direction === 'PENJUALAN' ? 'flow-sale' : 'flow-out');
        const itemText = (record.items || []).map(item => `${item.name} x${item.qty}`).join(', ');
        const nominal = record.amount > 0
            ? (normalizeTransactionPayType(record.payType) === 'Uang Putih' ? formatRP(record.amount) : formatUSD(record.amount))
            : '-';
        return `
            <tr>
                <td>${escapeHtml(record.time || '-')}</td>
                <td><span class="flow-badge ${badgeClass}">${escapeHtml(record.direction)}</span></td>
...
function drawStockFlowChart(days) {
    const canvas = document.getElementById('stock-flow-chart');
    if (!canvas) return;
    ...
    (window.stockFlowData || []).forEach(record => {
        const date = getStockFlowDate(record);
        const bucket = daily.get(localDateKey(date));
        if (!bucket) return;
        if (record.direction === 'MASUK') bucket.masuk += Number(record.totalQty) || 0;
        else bucket.keluar += Number(record.totalQty) || 0;
    });
    ...
    const top = 18;
    const bottom = 42;
    const chartW = parentWidth - left - right;
    ...
    const barW = Math.max(4, Math.min(18, groupW * 0.28));
    values.forEach((value, index) => {
        ...
        ctx.fillRect(center - barW - 2, top + chartH - inH, barW, inH);
        ctx.fillStyle = outColor;
        ctx.fillRect(center + 2, top + chartH - outH, barW, outH);
        ctx.fillStyle = textColor;
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.fillText(formatShortDate(dates[index]), center, top + chartH + 12);
    });
};

window.renderStockFlowDashboard = function() {
    const days = window.stockFlowChartDays || 7;
    const titleEl = document.getElementById('stock-flow-title');
    if (titleEl) titleEl.innerText = `Arus Stok Brangkas ${days} Hari Terakhir`;
    drawStockFlowChart(days);
    window.renderRecentStockActivity();
};
