...
const ADMIN_EMAIL_DOMAIN = "bmcc.local";

// GLOBAL STATE
// Login Pengurus sekarang memakai Firebase Authentication.
...
let adminSessionConfirmed = false;
...
function loadRememberedAdminLogin() {
    ...
    const rememberSessionInput = document.getElementById('remember-admin-password');
    if (!storage) return;

    const rememberId = storage.getItem(ADMIN_LOGIN_STORAGE_KEYS.rememberId) === '1';
    const rememberSession = storage.getItem(ADMIN_LOGIN_STORAGE_KEYS.rememberPassword) === '1';

    if (rememberIdInput) rememberIdInput.checked = rememberId;
    if (rememberSessionInput) rememberSessionInput.checked = rememberSession;
    if (idInput) {
        idInput.value = rememberId
            ? (storage.getItem(ADMIN_LOGIN_STORAGE_KEYS.adminId) || '')
            : '';
    }

    if (passwordInput) passwordInput.value = '';
    storage.removeItem(ADMIN_LOGIN_STORAGE_KEYS.password);
}

function saveRememberedAdminLogin(adminId) {
    const storage = getAdminLoginStorage();
    if (!storage) return;

    const rememberId = Boolean(document.getElementById('remember-admin-id')?.checked);
    const rememberSession = Boolean(document.getElementById('remember-admin-password')?.checked);

    storage.setItem(ADMIN_LOGIN_STORAGE_KEYS.rememberId, rememberId ? '1' : '0');
    storage.setItem(ADMIN_LOGIN_STORAGE_KEYS.rememberPassword, rememberSession ? '1' : '0');

    if (rememberId) {
        storage.setItem(ADMIN_LOGIN_STORAGE_KEYS.adminId, String(adminId || '').trim());
    } else {
        storage.removeItem(ADMIN_LOGIN_STORAGE_KEYS.adminId);
    }
...
function bindAdminRememberControls() {
    ...
    rememberSessionInput?.addEventListener('change', () => {
        if (rememberSessionInput.checked && rememberIdInput) {
...
function normalizeAdminEmail(value) {
    ...
    const safeId = raw.replace(/\s+/g, '').replace(/[^a-z0-9._-]/g, '');
    return safeId ? `${safeId}@${ADMIN_EMAIL_DOMAIN}` : '';
}

async function getAuthorizedAdminProfile(user) {
    ...
    return {
        ...
        name: String(profile.name || user.email || 'Pengurus'),
        role: 'admin',
        active: true
    };
}
...
function setAdminUiState(enabled) {
    window.isAdminLoggedIn = Boolean(enabled);
    document.querySelectorAll('.admin-only').forEach(el => {
        el.style.setProperty('display', enabled ? 'inline-block' : 'none', 'important');
    });
}

async function writeAuditLog(action, details = {}) {
    const user = auth.currentUser;
    if (!user || !currentAdminProfile) return;

    try {
        await addDoc(auditLogsRef, {
            actorUid: user.uid,
            actorEmail: user.email || currentAdminProfile.email || '',
            actorName: currentAdminProfile.name || 'Pengurus',
            action: String(action || 'UNKNOWN_ACTION'),
            targetPath: 'bmc_system/brangkas_data',
            details,
...
window.memberCatalogData = [
    ...
    { category: 'CLASS 3 (SENJATA +15%)', name: 'ASSAULT RIFFLE', priceBM: 625000, priceUP: 675000, note: 'Khusus Senjata +15%' },
...
async function verifyAdminCredentials(adminId, password) {
    const email = normalizeAdminEmail(adminId);
    if (!email || !password) return false;

    adminLoginInProgress = true;
    try {
        // Sesi hanya hidup selama halaman ini terbuka dan tidak dipulihkan otomatis.
        await setPersistence(auth, inMemoryPersistence);

        const credential = await signInWithEmailAndPassword(auth, email, String(password));
        const profile = await getAuthorizedAdminProfile(credential.user);
...
function ensureThemedToastSystem() {
    if (document.getElementById('bmc-toast-styles')) return;

    const style = document.createElement('style');
    style.id = 'bmc-toast-styles';
    style.textContent = `
        .bmc-toast-stack {
            position: fixed;
            right: 22px;
        ...
        .bmc-toast {
            ...
            background: linear-gradient(180deg, rgba(20,24,38,0.98), rgba(11,14,24,0.98));
        ...
        .bmc-toast.show { transform: translateY(0); opacity: 1; }
        .bmc-toast::before {
            content: '';
            position: absolute;
            left: 0;
        ...
        .bmc-toast.error::before { background: #ef4444; }
        .bmc-toast.warn::before { background: #f59e0b; }
        .bmc-toast.info::before { background: #3b82f6; }
        .bmc-toast .toast-icon {
            width: 36px; height: 36px; min-width: 36px;
            display: inline-flex; align-items: center; justify-content: center;
            border-radius: 12px;
            background: rgba(255, 196, 0, 0.12);
            color: #ffc83d; font-size: 18px; font-weight: 700;
        }
        .bmc-toast.error .toast-icon { background: rgba(239,68,68,.14); color: #ff7b7b; }
        .bmc-toast.warn .toast-icon { background: rgba(245,158,11,.14); color: #fbbf24; }
        .bmc-toast.info .toast-icon { background: rgba(59,130,246,.14); color: #60a5fa; }
        .bmc-toast .toast-content { flex: 1; min-width: 0; }
        .bmc-toast .toast-title { font-size: 14px; font-weight: 800; color: #f5b942; letter-spacing: .3px; margin-bottom: 4px; }
        .bmc-toast .toast-message { white-space: pre-line; line-height: 1.45; font-size: 13px; color: rgba(255,255,255,.92); }
        .bmc-toast .toast-close {
            border: 0; background: transparent; color: rgba(255,255,255,.54); cursor: pointer;
            font-size: 18px; line-height: 1; padding: 2px 4px; border-radius: 8px;
        }
        .bmc-toast .toast-close:hover { color: #fff; background: rgba(255,255,255,.06); }
        @media (max-width: 640px) {
            .bmc-toast-stack { left: 14px; right: 14px; bottom: 14px; width: auto; }
            .bmc-toast { padding: 12px 14px; border-radius: 14px; }
        }
    `;
    document.head.appendChild(style);

    const stack = document.createElement('div');
    stack.id = 'bmc-toast-stack';
    stack.className = 'bmc-toast-stack';
    document.body.appendChild(stack);
}

function getToastMeta(message) {
    const text = String(message || '').trim();
    if (/^✅|berhasil|success/i.test(text)) return { type: 'success', title: 'Berhasil', icon: '✓' };
    if (/^❌|gagal|ditolak|tidak dapat|tidak ditemukan|wajib|tidak mencukupi|error/i.test(text)) return { type: 'error', title: 'Gagal', icon: '✕' };
    if (/^⚠️|peringatan|warning|hati-hati|tunggu|terlalu banyak/i.test(text)) return { type: 'warn', title: 'Peringatan', icon: '!' };
    return { type: 'info', title: 'Info', icon: 'i' };
}

function showThemedToast(message, options = {}) {
    ensureThemedToastSystem();
    const stack = document.getElementById('bmc-toast-stack');
    if (!stack) return;

    const meta = { ...getToastMeta(message), ...(options || {}) };
    const toast = document.createElement('div');
    toast.className = `bmc-toast ${meta.type || 'info'}`;
    toast.innerHTML = `
        <div class="toast-icon">${meta.icon || 'i'}</div>
        <div class="toast-content">
            <div class="toast-title">${meta.title || 'Info'}</div>
            <div class="toast-message">${String(message || '').replace(/[<>]/g, m => m === '<' ? '&lt;' : '&gt;')}</div>
        </div>
    ...
    const duration = Number(options.duration) || (meta.type === 'error' ? 5200 : 3400);
...
window.handleRoleChange = async function() {
    ...
    if (role === 'admin') {
        // Jangan pernah memakai sesi lama untuk melewati form login.
        adminSessionConfirmed = false;
        currentAdminProfile = null;
        setAdminUiState(false);

        if (auth.currentUser) {
            await signOut(auth).catch(error => console.warn('Gagal membersihkan sesi lama:', error));
        }
...
window.submitAdminLogin = async function(event) {
    ...
    if (!adminId || !password) {
        return alert('Masukkan ID Admin/Email dan Password.');
    }
    ...
    try {
        ...
        if (!valid) throw new Error('UNAUTHORIZED_ADMIN');

        adminLoginFailedAttempts = 0;
        adminLoginLockedUntil = 0;
        saveRememberedAdminLogin(adminId);
        adminSessionConfirmed = true;
        ...
        await writeAuditLog('ADMIN_LOGIN_SUCCESS', {
            loginId: adminId.includes('@') ? adminId.toLowerCase() : adminId.toUpperCase()
        });

        window.switchTab('dashboard');
    } catch (error) {
        console.error('Login Pengurus gagal:', error);
        ...
        if (adminLoginFailedAttempts >= 5) {
            ...
            adminLoginFailedAttempts = 0;
            alert('❌ Login gagal. Percobaan dikunci selama 5 menit.');
        } else {
            alert(`❌ ID/email, password, atau hak admin salah. Percobaan ${adminLoginFailedAttempts}/5.`);
        }
    } finally {
        if (submitButton) {
...
window.cancelAdminAuth = async function() {
    document.getElementById('modal-pin')?.classList.remove('active');
    adminSessionConfirmed = false;
    currentAdminProfile = null;
    setAdminUiState(false);
...
window.deleteBrangkasItem = async function(vaultType, itemName) {
    if (!window.isAdminLoggedIn || !auth.currentUser) {
        return alert('❌ Login Pengurus diperlukan.');
    }
    if (!isInitialLoadComplete) {
        return alert('⏳ Tunggu sampai status Connected.');
    }
    ...
    if (items[normalizedName] === undefined) {
        return alert('❌ Item tidak ditemukan.');
    }
    if (!confirm(`Hapus ${normalizedName} dari ${getVaultLabel(normalizedVault)}?`)) return;
    ...
    const saved = await saveDataToCloud('DELETE_STOCK_ITEM', {
        vaultType: normalizedVault,
    ...
    alert(`✅ ${normalizedName} berhasil dihapus.`);
};
...
window.renderSalesData = function() {
    ...
    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="10" class="dashboard-empty">Belum ada data penjualan.</td></tr>';
        return;
    }
...
function buildHistoricalCashSnapshots() {
    ...
    cashEvents.sort((a, b) => {
        ...
        const priority = { MANUAL_ORDER: 0, STOCK_FLOW: 1, TRANSACTION: 2 };
...
function renderTransactionRows(tbodyId, transactionType) {
    ...
    tbody.innerHTML = filteredTransactions.map(tx => {
        const balanceAtThatTime = historicalCashSnapshots.get(tx._index);
...
async function saveDataToCloud(action = 'UPDATE_APP_DATA', details = {}) {
    if (!isInitialLoadComplete) {
        alert('⏳ Data server masih dimuat. Tunggu status Connected.');
        return false;
    }

    if (!adminSessionConfirmed || !auth.currentUser || !window.isAdminLoggedIn || !currentAdminProfile) {
        alert('❌ Sesi Pengurus tidak aktif. Login kembali.');
        return false;
    }
    ...
    try {
        let committedRevision = loadedRevision;

        await runTransaction(db, async transaction => {
            const serverSnap = await transaction.get(docRef);

            // Jangan pernah membuat database kosong otomatis.
            if (!serverSnap.exists()) {
                throw new Error('DATABASE_DOCUMENT_MISSING');
            }

            const serverData = serverSnap.data() || {};
            const serverRevision = Number(serverData.revision) || 0;

            // Browser/tab lama ditolak agar tidak menimpa data baru.
            if (serverRevision !== loadedRevision) {
            ...
            transaction.update(docRef, {
                ...
                lastUpdatedBy: {
                    ...
                    name: currentAdminProfile.name || 'Pengurus'
                }
            });
        });

        loadedRevision = committedRevision;
        await writeAuditLog(action, {
            ...details,
            revision: committedRevision
        });

        console.log(`✅ Data tersimpan. Revision ${committedRevision}.`);
        return true;
    } catch (error) {
        console.error('Gagal simpan ke Firestore:', error);

        if (error?.message === 'STALE_DATA') {
            alert('⚠️ Data sudah berubah dari perangkat/tab lain. Muat ulang halaman sebelum menyimpan.');
        } else if (error?.message === 'DATABASE_DOCUMENT_MISSING') {
            alert('❌ Dokumen database utama tidak ditemukan. Aplikasi tidak membuat data kosong otomatis.');
        } else if (String(error?.code || '').includes('permission-denied')) {
            alert('❌ Penulisan ditolak Firebase. Periksa akun Pengurus dan Rules.');
        } else {
            alert('❌ Data gagal disimpan. Periksa koneksi dan console browser.');
        }
...
function initAuthObserver() {
    onAuthStateChanged(auth, async user => {
        ...
        // Callback ini juga terpanggil saat signIn sedang berjalan.
        if (adminLoginInProgress) return;

        // Sesi yang dipulihkan browser atau tersisa dari versi lama tidak boleh membuka role Pengurus.
        if (!adminSessionConfirmed) {
            currentAdminProfile = null;
            setAdminUiState(false);
            await signOut(auth).catch(() => {});
            return;
        ...
        const profile = await getAuthorizedAdminProfile(user).catch(() => null);
        if (!profile) {
            adminSessionConfirmed = false;
            currentAdminProfile = null;
            setAdminUiState(false);
        ...
        currentAdminProfile = profile;
        const role = document.getElementById('user-role')?.value || 'member';
        setAdminUiState(role === 'admin');
    });
}
...
function initRealtimeSync() {
    ...
    onSnapshot(docRef, (docSnap) => {
        if (!docSnap.exists()) {
            if (statusEl) {
                statusEl.innerText = '⚠️ Database tidak ditemukan';
                statusEl.style.backgroundColor = '#f59e0b';
            }

            console.error(
                'Dokumen bmc_system/brangkas_data tidak ditemukan. ' +
                'Pembuatan data kosong otomatis dinonaktifkan.'
            );
        ...
        window.bmcToKelompokData = Array.isArray(data.bmcToKelompokData)
            ? data.bmcToKelompokData
            : window.initialBmcToKelompok;

        window.kelompokToBmcData = Array.isArray(data.kelompokToBmcData)
            ? data.kelompokToBmcData
            : window.initialKelompokToBmc;

        // Jika field hilang/salah format, jangan ubah menjadi array kosong.
        window.transactionsData = Array.isArray(data.transactionsData)
            ? data.transactionsData
            : window.transactionsData;

        window.orderHistoryData = Array.isArray(data.orderHistoryData)
            ? data.orderHistoryData
            : window.orderHistoryData;

        window.loanRecordsData = Array.isArray(data.loanRecordsData)
            ? data.loanRecordsData
            : window.loanRecordsData;

        window.stockFlowData = Array.isArray(data.stockFlowData)
            ? data.stockFlowData.map(record => ({
                ...record,
                vaultType: normalizeVaultType(record.vaultType || VAULT_BMC)
            }))
            : window.stockFlowData;
        ...
        // Migrasi lama hanya dilakukan di memori, tidak langsung menulis database.
        migrateLegacyOrderTransactions();
        migrateLegacyStockTransactions();
...
// =============================================================
// FIX: ORDER PENJUALAN ANGGOTA 20+ MASUK KE MODE PENGURUS
// Alur: Member checkout -> member_sales_orders (PENDING)
// Pengurus proses -> stok/saldo berubah -> sales (DONE)
// =============================================================
const memberSalesOrdersRef = collection(db, 'member_sales_orders');
const completedSalesRef = collection(db, 'sales');
let memberPendingOrders = [];
let completedSalesRecords = [];
let stopMemberOrderListener = null;
let stopCompletedSalesListener = null;
const autoProcessingMemberOrders = new Set();

function memberOrderDate(value, fallback = null) {
...
function formatMemberOrderDate(value, fallback = null) {
    const date = memberOrderDate(value, fallback);
...
function stopMemberSalesSync() {
    if (typeof stopMemberOrderListener === 'function') stopMemberOrderListener();
    if (typeof stopCompletedSalesListener === 'function') stopCompletedSalesListener();
    stopMemberOrderListener = null;
    stopCompletedSalesListener = null;
    memberPendingOrders = [];
    completedSalesRecords = [];
}

function startMemberSalesSync() {
    if (!window.isAdminLoggedIn || !auth.currentUser || !currentAdminProfile) return;
    if (stopMemberOrderListener || stopCompletedSalesListener) return;

    stopMemberOrderListener = onSnapshot(memberSalesOrdersRef, snapshot => {
        memberPendingOrders = snapshot.docs
            .map(orderDoc => ({ id: orderDoc.id, ...orderDoc.data() }))
            .filter(order => String(order.status || '').toUpperCase() === 'PENDING');
        window.renderSalesData();

        // Order member diproses otomatis. Pengurus tidak perlu menekan tombol apa pun.
        memberPendingOrders.forEach(order => {
            if (autoProcessingMemberOrders.has(order.id)) return;
            autoProcessingMemberOrders.add(order.id);
            window.processMemberPendingOrder(order.id, { automatic: true })
                .catch(error => console.error('Auto-process order gagal:', error))
                .finally(() => autoProcessingMemberOrders.delete(order.id));
        });
    }, error => {
        console.error('Gagal membaca order Penjualan Anggota 20+:', error);
    });

    stopCompletedSalesListener = onSnapshot(completedSalesRef, snapshot => {
        completedSalesRecords = snapshot.docs
            .map(saleDoc => ({ id: saleDoc.id, ...saleDoc.data() }));
        window.renderSalesData();
    }, error => {
        console.error('Gagal membaca riwayat penjualan baru:', error);
    });
}

setInterval(() => {
    if (window.isAdminLoggedIn && auth.currentUser && currentAdminProfile) {
        startMemberSalesSync();
    } else if (stopMemberOrderListener || stopCompletedSalesListener) {
        stopMemberSalesSync();
    }
}, 800);
...
window.checkoutMemberCart = async function() {
    ...
    const buyer = String(document.getElementById('cart-buyer-name')?.value || '').trim();
    const seller = String(document.getElementById('cart-seller-name')?.value || '').trim();
    const payCode = document.getElementById('cart-pay-type')?.value === 'BM' ? 'BM' : 'UP';

    if (!buyer || !seller) return alert('Mohon isi nama Pembeli dan Penjual!');
    if (buyer.length > 80 || seller.length > 80) {
        return alert('❌ Nama pembeli atau penjual terlalu panjang.');
    }

    const bossItems = getVaultItems(VAULT_BOSS);
    const shortages = window.cartItems.filter(item => {
        const itemName = normalizeItemName(item.name);
        const stockQty = convertSaleQtyToStockQty(item.name, item.qty);
        return (Number(bossItems[itemName]) || 0) < stockQty;
    });
    ...
    if (shortages.length > 0) {
        const details = shortages.map(item => {
            const itemName = normalizeItemName(item.name);
            const stockQty = convertSaleQtyToStockQty(item.name, item.qty);
            return `${itemName}: stok ${Number(bossItems[itemName]) || 0} PCS, diminta ${stockQty} PCS`;
        }).join('\n');
        return alert(`❌ Stok Brangkas Boss tidak mencukupi:\n\n${details}`);
    }

    let amount = 0;
    const orderItems = window.cartItems.map(item => {
        const saleQty = Math.max(1, Number(item.qty) || 1);
        const unitPrice = payCode === 'UP'
            ? Math.max(0, Number(item.priceUP) || 0)
            : Math.max(0, Number(item.priceBM) || 0);
        const stockQty = convertSaleQtyToStockQty(item.name, saleQty);
        amount += unitPrice * saleQty;

        return {
            name: normalizeItemName(item.name),
            qty: stockQty,
            saleQty,
            saleUnit: getMemberSaleUnitInfo(item.name).saleUnitLabel,
            unitPrice
        };
    });
    ...
    try {
        await addDoc(memberSalesOrdersRef, {
            buyer,
            seller,
            vaultType: VAULT_BOSS,
            items: orderItems,
            totalQty,
            amount,
            payType: payCode === 'UP' ? 'Uang Putih' : 'Black Money',
            status: 'PENDING',
            source: 'Penjualan Anggota 20+',
            schemaVersion: 1,
            clientTime: new Date().toISOString(),
            createdAt: serverTimestamp()
        });
        ...
        const sellerInput = document.getElementById('cart-seller-name');
        if (buyerInput) buyerInput.value = '';
        if (sellerInput) sellerInput.value = '';

        alert('✅ Penjualan berhasil dikirim dan langsung masuk ke Data Penjualan Pengurus.');
    } catch (error) {
        console.error('Gagal mengirim order anggota:', error);
        if (String(error?.code || '').includes('permission-denied')) {
            alert('❌ Firebase menolak order. Pasang firestore.rules terbaru dari paket ini.');
        } else {
            alert('❌ Order gagal dikirim. Periksa koneksi lalu coba lagi.');
        }
    }
};

window.processMemberPendingOrder = async function(orderId, options = {}) {
    if (!window.isAdminLoggedIn || !auth.currentUser || !currentAdminProfile) {
        return alert('❌ Login Pengurus diperlukan.');
    }

    const pendingOrder = memberPendingOrders.find(order => order.id === orderId);
    if (!pendingOrder) return;

    const orderRef = doc(db, 'member_sales_orders', orderId);
    const saleRef = doc(db, 'sales', orderId);

    try {
        await runTransaction(db, async transaction => {
            const mainSnap = await transaction.get(docRef);
            const orderSnap = await transaction.get(orderRef);

            if (!mainSnap.exists()) throw new Error('DATABASE_NOT_FOUND');
            if (!orderSnap.exists()) throw new Error('ORDER_NOT_FOUND');

            const mainData = mainSnap.data() || {};
            const orderData = orderSnap.data() || {};
            if (String(orderData.status || '').toUpperCase() !== 'PENDING') {
                throw new Error('ORDER_ALREADY_PROCESSED');
            }

            const state = JSON.parse(JSON.stringify(mainData.brangkasState || {}));
            state.whiteMoney = Number(state.whiteMoney) || 0;
            state.blackMoney = Number(state.blackMoney) || 0;
            state.redMoney = Number(state.redMoney) || 0;

            const legacyItems = normalizeItemObject(state.items || {});
            const sourceVaults = state.vaults && typeof state.vaults === 'object'
                ? state.vaults
                : {};
            state.vaults = {
                BMC: normalizeItemObject(sourceVaults.BMC || sourceVaults.bmc || legacyItems),
            ...
            state.items = state.vaults.BMC;

            const orderItems = mergeStockFlowItems(orderData.items || []);
            if (orderItems.length === 0) throw new Error('ORDER_EMPTY');

            const shortages = orderItems.filter(item => {
                const itemName = normalizeItemName(item.name);
                return (Number(state.vaults.BOSS[itemName]) || 0) < (Number(item.qty) || 0);
            });

            if (shortages.length > 0) {
                const shortageError = new Error('STOCK_SHORTAGE');
                shortageError.details = shortages.map(item => {
                    const itemName = normalizeItemName(item.name);
                    return `${itemName}: stok ${Number(state.vaults.BOSS[itemName]) || 0} PCS, diminta ${Number(item.qty) || 0} PCS`;
                }).join('\n');
                throw shortageError;
            }
            ...
            orderItems.forEach(item => {
                ...
                state.vaults.BOSS[itemName] = (Number(state.vaults.BOSS[itemName]) || 0) - qty;
                remainingStock[itemName] = state.vaults.BOSS[itemName];
            });

            const payType = normalizeTransactionPayType(orderData.payType);
            const amount = Math.max(0, Number(orderData.amount) || 0);
            if (payType === 'Black Money') state.blackMoney += amount;
            else state.whiteMoney += amount;
            ...
            const revision = (Number(mainData.revision) || 0) + 1;
            ...
            transaction.set(saleRef, {
                orderId,
                buyer: String(orderData.buyer || '-'),
                seller: String(orderData.seller || '-'),
                vaultType: VAULT_BOSS,
                items: orderItems,
                ...
                source: 'Penjualan Anggota 20+',
                remainingStock,
                orderedAt: orderData.createdAt || null,
                orderedClientTime: orderData.clientTime || null,
        ...
        await writeAuditLog('PROCESS_MEMBER_SALE_AUTO', { orderId });
        if (!options.automatic) {
            alert('✅ Order berhasil diproses. Stok dan saldo sudah diperbarui.');
        }
    } catch (error) {
        console.error('Gagal memproses order:', error);
        if (options.automatic) {
            console.warn('Order belum dapat diproses otomatis:', error?.message, error?.details || '');
            return;
        }
        if (error?.message === 'STOCK_SHORTAGE') {
            alert(`❌ Stok Brangkas Boss tidak mencukupi:\n\n${error.details || '-'}`);
        } else if (error?.message === 'ORDER_ALREADY_PROCESSED') {
            alert('⚠️ Order ini sudah diproses sebelumnya.');
        } else {
            alert('❌ Order gagal diproses. Periksa Rules dan koneksi Firebase.');
        }
    }
};

window.rejectMemberPendingOrder = async function(orderId) {
    if (!window.isAdminLoggedIn || !auth.currentUser || !currentAdminProfile) {
        return alert('❌ Login Pengurus diperlukan.');
    }

    const pendingOrder = memberPendingOrders.find(order => order.id === orderId);
    if (!pendingOrder) return alert('❌ Order PENDING tidak ditemukan.');
    if (!confirm(`Tolak order dari ${pendingOrder.buyer || '-'}?`)) return;
    ...
    try {
        await runTransaction(db, async transaction => {
            const orderSnap = await transaction.get(orderRef);
            if (!orderSnap.exists()) throw new Error('ORDER_NOT_FOUND');
            if (String(orderSnap.data()?.status || '').toUpperCase() !== 'PENDING') {
                throw new Error('ORDER_ALREADY_PROCESSED');
            }
        ...
        await writeAuditLog('REJECT_MEMBER_SALE', { orderId });
        alert('✅ Order berhasil ditolak.');
    } catch (error) {
        console.error('Gagal menolak order:', error);
        alert('❌ Order gagal ditolak.');
    }
};


let completedSaleEditDraft = [];

function ensureCompletedSaleEditModal() {
    if (document.getElementById('modal-edit-completed-sale')) return;

    const modal = document.createElement('div');
    modal.id = 'modal-edit-completed-sale';
    modal.className = 'modal';
    modal.innerHTML = `
        <div class="modal-content" style="max-width:820px;">
            <div class="modal-header">
                <h3 style="margin:0; color:var(--accent-gold);">✏️ Edit Data Penjualan</h3>
                <button type="button" class="btn btn-sm btn-red" onclick="window.closeCompletedSaleEdit()">×</button>
            </div>
            <form onsubmit="window.saveCompletedSaleEdit(event)">
                <input type="hidden" id="completed-sale-edit-id">
                <div class="form-row">
                    <div class="form-group">
                        <label>Pembeli</label>
                        <input id="completed-sale-edit-buyer" class="form-control" maxlength="80" required>
                    </div>
                    <div class="form-group">
                        <label>Penjual</label>
                        <input id="completed-sale-edit-seller" class="form-control" maxlength="80" required>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>Pembayaran</label>
                        <select id="completed-sale-edit-paytype" class="form-control">
                            <option value="Uang Putih">Uang Putih</option>
                            <option value="Black Money">Black Money</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Nominal</label>
                        <input id="completed-sale-edit-amount" type="number" min="0" class="form-control" required>
                    </div>
                </div>

                <div style="margin:14px 0 8px; font-weight:700; color:var(--accent-gold);">Barang (PCS)</div>
                <div id="completed-sale-edit-items"></div>
                <button type="button" class="btn btn-sm btn-blue" onclick="window.addCompletedSaleEditItem()" style="margin-top:8px;">+ Tambah Barang</button>

                <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:18px;">
                    <button type="button" class="btn" onclick="window.closeCompletedSaleEdit()">Batal</button>
...
function getCompletedSaleById(saleId) {
    return completedSalesRecords.find(sale => sale.id === saleId) || null;
}

function renderCompletedSaleEditItems() {
    const container = document.getElementById('completed-sale-edit-items');
    if (!container) return;

    if (!completedSaleEditDraft.length) {
        completedSaleEditDraft.push({ name: '', qty: 1 });
    }

    container.innerHTML = completedSaleEditDraft.map((item, index) => `
        <div style="display:grid; grid-template-columns:minmax(0,1fr) 130px auto; gap:8px; align-items:end; margin-bottom:8px;">
            <div class="form-group" style="margin:0;">
                <label>Nama Barang</label>
                <input class="form-control" value="${escapeHtml(item.name)}" onchange="window.updateCompletedSaleEditItem(${index}, 'name', this.value)" required>
            </div>
            <div class="form-group" style="margin:0;">
                <label>QTY PCS</label>
                <input type="number" min="1" class="form-control" value="${Math.max(1, Number(item.qty)||1)}" onchange="window.updateCompletedSaleEditItem(${index}, 'qty', this.value)" required>
            </div>
            <button type="button" class="btn btn-sm btn-red" onclick="window.removeCompletedSaleEditItem(${index})">Hapus</button>
        </div>`).join('');
}

window.openCompletedSaleEdit = function(saleId) {
    if (!window.isAdminLoggedIn || !auth.currentUser || !currentAdminProfile) {
        return alert('❌ Login Pengurus diperlukan.');
    }

    const sale = getCompletedSaleById(saleId);
    if (!sale) return alert('❌ Data penjualan tidak ditemukan.');

    ensureCompletedSaleEditModal();
    document.getElementById('completed-sale-edit-id').value = saleId;
    document.getElementById('completed-sale-edit-buyer').value = sale.buyer || '';
    document.getElementById('completed-sale-edit-seller').value = sale.seller || '';
    document.getElementById('completed-sale-edit-paytype').value = normalizeTransactionPayType(sale.payType) === 'Black Money' ? 'Black Money' : 'Uang Putih';
    document.getElementById('completed-sale-edit-amount').value = Math.max(0, Number(sale.amount) || 0);
    completedSaleEditDraft = mergeStockFlowItems(sale.items || []).map(item => ({ name: item.name, qty: item.qty }));
    renderCompletedSaleEditItems();
    document.getElementById('modal-edit-completed-sale').classList.add('active');
};

window.closeCompletedSaleEdit = function() {
    document.getElementById('modal-edit-completed-sale')?.classList.remove('active');
};

window.addCompletedSaleEditItem = function() {
    completedSaleEditDraft.push({ name: '', qty: 1 });
    renderCompletedSaleEditItems();
};

window.updateCompletedSaleEditItem = function(index, field, value) {
    const item = completedSaleEditDraft[index];
    if (!item) return;
    if (field === 'name') item.name = normalizeItemName(value);
    if (field === 'qty') item.qty = Math.max(1, Number.parseInt(value, 10) || 1);
};

window.removeCompletedSaleEditItem = function(index) {
    completedSaleEditDraft.splice(index, 1);
    renderCompletedSaleEditItems();
};

window.saveCompletedSaleEdit = async function(event) {
    if (event) event.preventDefault();
    if (!window.isAdminLoggedIn || !auth.currentUser || !currentAdminProfile) {
        return alert('❌ Login Pengurus diperlukan.');
    }

    const saleId = String(document.getElementById('completed-sale-edit-id')?.value || '');
    const saleRef = doc(db, 'sales', saleId);
    const buyer = String(document.getElementById('completed-sale-edit-buyer')?.value || '').trim();
    const seller = String(document.getElementById('completed-sale-edit-seller')?.value || '').trim();
    const newPayType = document.getElementById('completed-sale-edit-paytype')?.value === 'Black Money' ? 'Black Money' : 'Uang Putih';
    const newAmount = Math.max(0, Number(document.getElementById('completed-sale-edit-amount')?.value) || 0);
    const newItems = mergeStockFlowItems(completedSaleEditDraft);

    if (!buyer || !seller) return alert('❌ Pembeli dan Penjual wajib diisi.');
    if (!newItems.length) return alert('❌ Minimal satu barang wajib diisi.');
    ...
    try {
        ...
        await runTransaction(db, async transaction => {
            ...
            if (!mainSnap.exists() || !saleSnap.exists()) throw new Error('DATA_NOT_FOUND');

            const mainData = mainSnap.data() || {};
            const saleData = saleSnap.data() || {};
            oldSnapshot = saleData;
            ...
            state.redMoney = Number(state.redMoney) || 0;
            const legacyItems = normalizeItemObject(state.items || {});
            const sourceVaults = state.vaults || {};
            state.vaults = {
                BMC: normalizeItemObject(sourceVaults.BMC || sourceVaults.bmc || legacyItems),
                BOSS: normalizeItemObject(sourceVaults.BOSS || sourceVaults.boss || {})
            };
            ...
            const oldItems = mergeStockFlowItems(saleData.items || []);
            oldItems.forEach(item => {
                const name = normalizeItemName(item.name);
                state.vaults.BOSS[name] = (Number(state.vaults.BOSS[name]) || 0) + (Number(item.qty) || 0);
            });

            const shortages = newItems.filter(item => {
                const name = normalizeItemName(item.name);
                return (Number(state.vaults.BOSS[name]) || 0) < (Number(item.qty) || 0);
            });
            if (shortages.length) {
                const err = new Error('STOCK_SHORTAGE');
                err.details = shortages.map(item => {
                    const name = normalizeItemName(item.name);
                    return `${name}: tersedia ${Number(state.vaults.BOSS[name]) || 0} PCS, dibutuhkan ${Number(item.qty) || 0} PCS`;
                }).join('\n');
                throw err;
            }
            ...
            const oldPayType = normalizeTransactionPayType(saleData.payType);
            const oldAmount = Math.max(0, Number(saleData.amount) || 0);
            const oldKey = oldPayType === 'Black Money' ? 'blackMoney' : 'whiteMoney';
            const newKey = newPayType === 'Black Money' ? 'blackMoney' : 'whiteMoney';

            if (oldKey === newKey) {
                const adjusted = (Number(state[oldKey]) || 0) - oldAmount + newAmount;
                if (adjusted < 0) throw new Error('CASH_SHORTAGE');
                state[oldKey] = adjusted;
            } else {
                const oldBalance = (Number(state[oldKey]) || 0) - oldAmount;
                if (oldBalance < 0) throw new Error('CASH_SHORTAGE');
                state[oldKey] = oldBalance;
                state[newKey] = (Number(state[newKey]) || 0) + newAmount;
            }

            const remainingStock = {};
            newItems.forEach(item => {
                const name = normalizeItemName(item.name);
                remainingStock[name] = Number(state.vaults.BOSS[name]) || 0;
            });
            ...
            committedRevision = (Number(mainData.revision) || 0) + 1;
            ...
            transaction.update(saleRef, {
                ...
                seller,
                items: newItems,
                totalQty: newItems.reduce((sum, item) => sum + (Number(item.qty) || 0), 0),
                payType: newPayType,
                amount: newAmount,
                remainingStock,
        ...
        window.brangkasState = committedState || window.brangkasState;
        loadedRevision = committedRevision;
        ensureVaultState();
        await writeAuditLog('EDIT_COMPLETED_SALE', {
            saleId,
            oldAmount: Math.max(0, Number(oldSnapshot?.amount) || 0),
            newAmount,
            revision: committedRevision
        });
        renderAll();
        window.closeCompletedSaleEdit();
        alert('✅ Data penjualan berhasil diedit. Stok dan saldo sudah dikoreksi.');
    } catch (error) {
        console.error('Gagal edit penjualan:', error);
        if (error?.message === 'STOCK_SHORTAGE') {
            alert(`❌ Stok tidak mencukupi untuk perubahan:\n\n${error.details || '-'}`);
        } else if (error?.message === 'CASH_SHORTAGE') {
            alert('❌ Saldo saat ini tidak cukup untuk membalik nominal transaksi lama.');
        } else {
...
window.deleteCompletedSale = async function(saleId) {
    if (!window.isAdminLoggedIn || !auth.currentUser || !currentAdminProfile) {
        return alert('❌ Login Pengurus diperlukan.');
    }
    ...
    const sale = getCompletedSaleById(saleId);
    if (!sale) return alert('❌ Data penjualan tidak ditemukan.');
    if (!confirm(`Hapus penjualan ${sale.buyer || '-'}? Barang akan kembali ke Brangkas Boss dan saldo akan dikoreksi.`)) return;

    const saleRef = doc(db, 'sales', saleId);
    const orderRef = doc(db, 'member_sales_orders', sale.orderId || saleId);
    ...
    try {
        ...
        await runTransaction(db, async transaction => {
            ...
            mergeStockFlowItems(saleData.items || []).forEach(item => {
                const name = normalizeItemName(item.name);
                state.vaults.BOSS[name] = (Number(state.vaults.BOSS[name]) || 0) + (Number(item.qty) || 0);
            });

            const payType = normalizeTransactionPayType(saleData.payType);
            const amount = Math.max(0, Number(saleData.amount) || 0);
            const moneyKey = payType === 'Black Money' ? 'blackMoney' : 'whiteMoney';
            const correctedBalance = (Number(state[moneyKey]) || 0) - amount;
            if (correctedBalance < 0) throw new Error('CASH_SHORTAGE');
            state[moneyKey] = correctedBalance;
            ...
            // Hapus riwayat final dari Data Penjualan.
            transaction.delete(saleRef);

            // Tandai order asal agar tidak pernah diproses ulang otomatis.
            transaction.update(orderRef, {
                status: 'DELETED',
        ...
        loadedRevision = committedRevision;
        ensureVaultState();
        await writeAuditLog('DELETE_COMPLETED_SALE', {
            saleId,
            buyer: sale.buyer || '-',
        ...
        renderAll();
        alert('✅ Data penjualan dihapus. Barang dikembalikan dan saldo sudah dikoreksi.');
    } catch (error) {
        console.error('Gagal hapus penjualan:', error);
        if (error?.message === 'CASH_SHORTAGE') {
            alert('❌ Saldo saat ini tidak cukup untuk menghapus transaksi ini karena nominal penjualan harus dikurangi dari saldo.');
        } else {
            alert('❌ Data penjualan gagal dihapus. Periksa Firebase Rules terbaru.');
        }
    }
};

const renderSalesDataBeforeMemberQueue = window.renderSalesData;
window.renderSalesData = function() {
    renderSalesDataBeforeMemberQueue();

    const tbody = document.getElementById('tbody-sales-data');
    if (!tbody || !window.isAdminLoggedIn) return;

    const newRows = [];

    memberPendingOrders
        .slice()
        .sort((a, b) => memberOrderDate(b.createdAt, b.clientTime) - memberOrderDate(a.createdAt, a.clientTime))
        .forEach(order => {
            const itemText = (order.items || [])
                .map(item => `${item.name} x${Number(item.qty) || 0}`)
                .join(', ');
            const payType = normalizeTransactionPayType(order.payType);
            const nominal = payType === 'Uang Putih' ? formatRP(order.amount) : formatUSD(order.amount);

            newRows.push(`
                <tr>
                    <td>${escapeHtml(formatMemberOrderDate(order.createdAt, order.clientTime))}</td>
                    <td><span class="badge badge-black">Brangkas Boss</span></td>
                    <td class="stock-activity-notes">${escapeHtml(`Pembeli: ${order.buyer || '-'} | Penjual: ${order.seller || '-'}`)}</td>
                    <td>${escapeHtml(itemText || '-')}</td>
                    <td>${Number(order.totalQty) || 0} PCS</td>
                    <td><span class="badge badge-black">${escapeHtml(payType)}</span></td>
                    <td>${escapeHtml(nominal)}</td>
                    <td><span class="badge badge-black">MEMPROSES</span></td>
                    <td><span style="color:var(--text-muted);">Otomatis</span></td>
                    <td><span class="badge badge-black">AUTO</span></td>
                </tr>`);
        });

    completedSalesRecords
        .slice()
        .sort((a, b) => memberOrderDate(b.processedAt, b.orderedAt || b.orderedClientTime) - memberOrderDate(a.processedAt, a.orderedAt || a.orderedClientTime))
        .forEach(sale => {
            const itemText = (sale.items || [])
                .map(item => `${item.name} x${Number(item.qty) || 0}`)
                .join(', ');
            const payType = normalizeTransactionPayType(sale.payType);
            const nominal = payType === 'Uang Putih' ? formatRP(sale.amount) : formatUSD(sale.amount);
            const remainingStock = sale.remainingStock || {};
            const stockHtml = (sale.items || []).map(item => {
                const itemName = normalizeItemName(item.name);
                const qty = Number(remainingStock[itemName]) || 0;
                const badgeClass = qty <= 30 ? 'badge-red' : 'badge-green';
                return `<div style="margin-bottom:4px;"><span class="badge ${badgeClass}">${escapeHtml(itemName)}: ${qty} PCS</span></div>`;
            }).join('') || '-';

            newRows.push(`
                <tr>
                    <td>${escapeHtml(formatMemberOrderDate(sale.processedAt, sale.orderedAt || sale.orderedClientTime))}</td>
                    <td><span class="badge badge-black">Brangkas Boss</span></td>
                    <td class="stock-activity-notes">${escapeHtml(`Pembeli: ${sale.buyer || '-'} | Penjual: ${sale.seller || '-'}`)}</td>
                    <td>${escapeHtml(itemText || '-')}</td>
                    <td>${Number(sale.totalQty) || 0} PCS</td>
                    <td><span class="badge badge-black">${escapeHtml(payType)}</span></td>
                    <td>${escapeHtml(nominal)}</td>
                    <td><span class="badge badge-green">DONE</span></td>
                    <td><div class="movement-item-lines">${stockHtml}</div></td>
                    <td>
                        <div class="sales-action-buttons">
                            <button class="btn btn-sm btn-blue" type="button" onclick="window.openCompletedSaleEdit('${sale.id}')">Edit</button>
                            <button class="btn btn-sm btn-red" type="button" onclick="window.deleteCompletedSale('${sale.id}')">Hapus</button>
                        </div>
                    </td>
                </tr>`);
        });
    ...
    if (newRows.length > 0) {
        tbody.querySelectorAll('.dashboard-empty').forEach(cell => cell.closest('tr')?.remove());
