...
window.addLossRecord = async function() {
    ...
    const qty = Math.max(1, Number.parseInt(document.getElementById('loss-item-qty')?.value, 10) || 0);
    const reason = String(document.getElementById('loss-reason')?.value || '').trim() || 'Loss Barang';
    ...
    if (!qty) return alert('❌ Jumlah loss wajib diisi.');
    ...
    renderAll();
    document.getElementById('loss-item-qty').value = '';
    document.getElementById('loss-reason').value = '';
    document.getElementById('loss-note').value = '';
    window.renderLossItemOptions();
    alert(`✅ Loss ${item} x${qty} dari ${getVaultLabel(vaultType)} berhasil dicatat.`);
};
...
window.deleteLossRecord = async function(lossId) {
    ...
    if (!confirm(`Hapus data loss ${record.item} x${record.qty}?`)) return;
    ...
    await saveDataToCloud();
    renderAll();
    alert('✅ Data loss dihapus dan stok dikembalikan.');
};
...
window.renderLossRecords = function() {
    ...
    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; color:var(--text-muted);">Belum ada data loss barang.</td></tr>';
        return;
    }
    ...
    records.forEach(record => {
        ...
        html += `
            <tr>
                ...
                <td><span class="badge badge-black">${escapeHtml(getVaultLabel(record.vaultType))}</span></td>
                <td>${escapeHtml(record.item)}</td>
                <td>${record.qty} PCS</td>
                <td><span class="badge badge-blue">${record.beforeQty} PCS</span></td>
                <td><span class="badge badge-red">${record.afterQty} PCS</span></td>
