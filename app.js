...
window.renderSalesData = function() {
    ...
    const records = [...(window.stockFlowData || [])]
        .filter(record => record.direction === 'PENJUALAN')
        .sort((a, b) => getStockFlowDate(b) - getStockFlowDate(a))
        .slice(0, 30);

    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="dashboard-empty">Belum ada data penjualan.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = records.map(record => {
        ...
        const nominal = payType === 'Uang Putih'
...
function buildStockMovementRows(direction) {
    ...
    if (records.length === 0) {
        ...
        return `<tr><td colspan="6" class="dashboard-empty">Belum ada ${label}.</td></tr>`;
    }
