...
import { getFirestore, doc, setDoc, updateDoc, deleteField, FieldPath, onSnapshot, runTransaction } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
let hasLoadedExistingCloudDocument = false;
let lastLoadedCloudRevision = 0;
let lastLoadedCloudUpdated = null;
let lastLoadedServerData = null;
...
window.memberCatalogData = [
    ...
    { category: 'CLASS 1 (SENJATA +15%)', name: 'CERAMIC', priceBM: 85000, priceUP: 94000, note: 'Khusus Senjata +15%' },
    { category: 'CLASS 1 (SENJATA +15%)', name: 'REVOLVER MK2', priceBM: 162000, priceUP: 176000, note: 'Khusus Senjata +15%' },
    { category: 'CLASS 2 (SENJATA +15%)', name: 'SMG', priceBM: 264000, priceUP: 286000, note: 'Khusus Senjata +15%' },
    { category: 'CLASS 2 (SENJATA +15%)', name: 'MICRO SMG', priceBM: 264000, priceUP: 286000, note: 'Khusus Senjata +15%' },
    { category: 'CLASS 2 (SENJATA +15%)', name: 'MINI SMG', priceBM: 246000, priceUP: 267000, note: 'Khusus Senjata +15%' },
    { category: 'CLASS 2 (SENJATA +15%)', name: 'TEC-9', priceBM: 246000, priceUP: 267000, note: 'Khusus Senjata +15%' },
    { category: 'CLASS 3 (SENJATA +15%)', name: 'ASSAULT RIFFLE', priceBM: 474000, priceUP: 514000, note: 'Khusus Senjata +15%' },
    { category: 'CLASS 3 (SENJATA +15%)', name: 'SPECIAL CARBINE', priceBM: 588000, priceUP: 637000, note: 'Khusus Senjata +15%' },
...
window.initialBmcToKelompok = [
    { group: 'HAKUSHIKAI', category: 'VEST', item: 'VEST', qty: '30-40 PCS', priceWO: 120000, priceW: 0, note: '-', ket: '-' },
    ...
    { group: 'H2', category: 'VEST', item: 'VEST', qty: '50 PCS', priceWO: 110000, priceW: 0, note: '-', ket: '-' },
...
window.initialKelompokToBmc = [
    { group: 'HAKUSHIKAI', category: 'CLASS 1', item: 'CERAMIC', qty: '30 PCS', priceWO: 72000, priceW: 0, note: '-', ket: 'TBA' },
    { group: 'HAKUSHIKAI', category: 'CLASS 1', item: 'REVOLVER', qty: '30 PCS', priceWO: 135000, priceW: 175000, note: '-', ket: '-' },
    { group: 'HAKUSHIKAI', category: 'CLASS 2', item: 'TEC - 9', qty: '30 PCS', priceWO: 205000, priceW: 200000, note: '-', ket: '-' },
    { group: 'HAKUSHIKAI', category: 'CLASS 2', item: 'MINI SMG', qty: '30 PCS', priceWO: 205000, priceW: 200000, note: '-', ket: '-' },
    { group: 'HAKUSHIKAI', category: 'CLASS 2', item: 'MICRO SMG', qty: '30 PCS', priceWO: 220000, priceW: 240000, note: '-', ket: '6 KOTAK KARET, 7 SPRING, 6 GEAR' },
    { group: 'HAKUSHIKAI', category: 'CLASS 2', item: 'SMG', qty: '30 PCS', priceWO: 220000, priceW: 240000, note: '-', ket: '8 KOTAK KARET, 8 SPRING, 7 GEAR' },
    { group: 'HAKUSHIKAI', category: 'CLASS 3', item: 'ASSAULT RIFFLE', qty: '3 PCS', priceWO: 395000, priceW: 0, note: '-', ket: 'TBA' },
    { group: 'HAKUSHIKAI', category: 'CLASS 3', item: 'SPECIAL CARBINE', qty: '3 PCS', priceWO: 490000, priceW: 0, note: '-', ket: 'TBA' },
...
window.deleteBrangkasItem = async function(vaultType, itemName) {
    ...
    // Semua perubahan stok sekarang masuk melalui saveData() transaction-safe.
    // Ini mencegah delete langsung melewati revision guard dan menimpa perubahan perangkat lain.
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button
                            ...
                            type="button"
                            onclick="window.deleteOrderHistory(${order._index})"
                        >
                            Hapus
                        </button>
                    </td>
...
window.deleteOrderHistory = async function(orderIndex) {
    if (!window.isAdminLoggedIn) {
        return alert('❌ Hanya pengurus yang dapat menghapus riwayat order.');
    }

    const order = (window.orderHistoryData || [])[orderIndex];
    if (!order) return alert('❌ Data order tidak ditemukan.');

    const label = `${order.group || '-'} | ${order.item || '-'} | ${order.qty || 0} PCS`;
    if (!confirm(`Hapus riwayat order ${label}?`)) return;

    window.orderHistoryData.splice(orderIndex, 1);
    await window.saveData();
    renderAll();
};
...
// BACKUP KESELAMATAN LOKAL + PROTEKSI ANTI-OVERWRITE
const BMC_BACKUP_RING_KEY = 'bmc_safety_backups_v2';
const BMC_LEGACY_BACKUP_KEY = 'bmc_safety_backup_latest';
const BMC_BACKUP_LIMIT = 12;
...
function getHistoryFootprint(source) {
    const data = source || {};
    const brangkas = data.brangkasState || {};
    const vaults = brangkas.vaults || {};
    const vaultItemCount = Object.keys(vaults.BMC || {}).length + Object.keys(vaults.BOSS || {}).length;
    return vaultItemCount
        + (Array.isArray(data.transactionsData) ? data.transactionsData.length : 0)
        + (Array.isArray(data.orderHistoryData) ? data.orderHistoryData.length : 0)
        + (Array.isArray(data.loanRecordsData) ? data.loanRecordsData.length : 0)
        + (Array.isArray(data.lossRecordsData) ? data.lossRecordsData.length : 0)
        + (Array.isArray(data.stockFlowData) ? data.stockFlowData.length : 0);
}

function getBackupRing() {
    try {
        const parsed = JSON.parse(localStorage.getItem(BMC_BACKUP_RING_KEY) || '[]');
        return Array.isArray(parsed) ? parsed : [];
    } catch (error) {
        console.warn('⚠️ Backup ring tidak dapat dibaca:', error);
        return [];
    }
}

function saveLocalSafetyBackup(source = 'manual', explicitData = null) {
    try {
        const entry = {
            savedAt: new Date().toISOString(),
            source,
            data: cloneJson(explicitData || getCurrentAppDataSnapshot())
        };

        // Tetap isi key lama agar backup dari versi sebelumnya tetap kompatibel.
        localStorage.setItem(BMC_LEGACY_BACKUP_KEY, JSON.stringify(entry));

        const ring = getBackupRing();
        ring.unshift(entry);
        localStorage.setItem(BMC_BACKUP_RING_KEY, JSON.stringify(ring.slice(0, BMC_BACKUP_LIMIT)));
        return entry;
    } catch (error) {
...
function getAllLocalBackups() {
    const candidates = [...getBackupRing()];
    try {
        const legacy = JSON.parse(localStorage.getItem(BMC_LEGACY_BACKUP_KEY) || 'null');
        if (legacy?.data) candidates.push(legacy);
    } catch (_) {}
    ...
    return candidates
        ...
        .filter(entry => {
            const key = `${entry.savedAt || ''}|${entry.source || ''}|${getHistoryFootprint(entry.data)}`;
            if (seen.has(key)) return false;
            seen.add(key);
            return true;
        })
        .sort((a, b) => getHistoryFootprint(b.data) - getHistoryFootprint(a.data));
}

function getBestLocalBackup() {
    return getAllLocalBackups()[0] || null;
}
...
function buildCloudPayload() {
    ...
    return {
        brangkasState: cloneJson(window.brangkasState),
        bmcToKelompokData: cloneJson(window.bmcToKelompokData),
        kelompokToBmcData: cloneJson(window.kelompokToBmcData),
        transactionsData: cloneJson(window.transactionsData),
        orderHistoryData: cloneJson(window.orderHistoryData),
        loanRecordsData: cloneJson(window.loanRecordsData),
        lossRecordsData: cloneJson(window.lossRecordsData),
        stockFlowData: cloneJson(window.stockFlowData)
    };
}

function applyCloudData(data) {
    window._vaultMigrationNeeded = !data.brangkasState?.vaults;
    window.brangkasState = data.brangkasState || { whiteMoney: 0, blackMoney: 0, redMoney: 0, items: {}, vaults: { BMC: {}, BOSS: {} } };
    ensureVaultState();
    ...
    syncPriceMasterData();
    window.transactionsData = Array.isArray(data.transactionsData) ? data.transactionsData : [];
    window.orderHistoryData = Array.isArray(data.orderHistoryData) ? data.orderHistoryData : [];
...
function ensureRecoveryUiStyles() {
    if (document.getElementById('bmc-recovery-inline-style')) return;
    const style = document.createElement('style');
    style.id = 'bmc-recovery-inline-style';
    style.textContent = `
      .bmc-recovery-banner{position:fixed;left:50%;bottom:18px;transform:translateX(-50%);z-index:99990;width:min(760px,calc(100vw - 28px));background:#0b0b0d;border:1px solid #7f1d1d;box-shadow:0 20px 55px rgba(0,0,0,.65),0 0 28px rgba(220,38,38,.12);padding:14px 16px;display:flex;gap:14px;align-items:center;justify-content:space-between;font-family:Rajdhani,sans-serif}
      .bmc-recovery-copy{min-width:0}.bmc-recovery-title{font-weight:900;color:#f4f4f5;letter-spacing:.05em}.bmc-recovery-text{margin-top:3px;color:#a1a1aa;font-size:13px;font-weight:600}.bmc-recovery-actions{display:flex;gap:8px;flex-wrap:wrap}
      .bmc-recovery-btn{border:1px solid #3f3f46;background:#151518;color:#f4f4f5;padding:9px 13px;font:800 12px/1 Rajdhani,sans-serif;letter-spacing:.06em;cursor:pointer}.bmc-recovery-btn.primary{border-color:#b91c1c;background:#2a0d0f;color:#fecaca}.bmc-recovery-btn:hover{filter:brightness(1.18)}
      .bmc-recovery-modal{position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.78);display:grid;place-items:center;padding:18px}.bmc-recovery-card{width:min(760px,100%);max-height:82vh;overflow:auto;background:#0c0c0f;border:1px solid #3f3f46;box-shadow:0 35px 90px rgba(0,0,0,.8);padding:20px}.bmc-recovery-card h3{margin:0 0 6px;color:#f4f4f5;font:900 22px Rajdhani,sans-serif}.bmc-recovery-card p{color:#a1a1aa;font:600 13px/1.5 Rajdhani,sans-serif}.bmc-backup-row{border:1px solid #27272a;background:#111114;padding:12px;margin-top:10px;display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center}.bmc-backup-meta{font:700 13px/1.45 Rajdhani,sans-serif;color:#e4e4e7}.bmc-backup-sub{color:#71717a;font-size:12px}.bmc-recovery-footer{display:flex;justify-content:flex-end;gap:8px;margin-top:16px}
      @media(max-width:640px){.bmc-recovery-banner{align-items:flex-start;flex-direction:column}.bmc-backup-row{grid-template-columns:1fr}.bmc-recovery-actions{width:100%}.bmc-recovery-actions .bmc-recovery-btn{flex:1}}
    `;
    document.head.appendChild(style);
}

function downloadBackupJson(entry) {
    const blob = new Blob([JSON.stringify(entry, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `bmc-backup-${String(entry.savedAt || Date.now()).replaceAll(':', '-')}.json`;
...
window.showBmcRecoveryModal = function() {
    ensureRecoveryUiStyles();
    const backups = getAllLocalBackups();
    document.getElementById('bmc-recovery-modal')?.remove();
    const overlay = document.createElement('div');
    overlay.id = 'bmc-recovery-modal';
    overlay.className = 'bmc-recovery-modal';
    overlay.innerHTML = `
      <div class="bmc-recovery-card">
        <h3>BACKUP & RECOVERY</h3>
        <p>Pilih backup lokal yang ingin dipulihkan. Sebelum restore, kondisi cloud saat ini akan disimpan lagi ke backup lokal.</p>
        <div id="bmc-backup-list"></div>
        <div class="bmc-recovery-footer"><button class="bmc-recovery-btn" type="button" id="bmc-recovery-close">TUTUP</button></div>
      </div>`;
    document.body.appendChild(overlay);
    const list = overlay.querySelector('#bmc-backup-list');
    if (!backups.length) {
        list.innerHTML = '<p>Tidak ada backup lokal yang ditemukan di browser ini.</p>';
    } else {
        backups.forEach((entry, index) => {
            const row = document.createElement('div');
            const balances = entry.data?.brangkasState || {};
            row.className = 'bmc-backup-row';
            row.innerHTML = `
              <div class="bmc-backup-meta">
                ${escapeHtml(new Date(entry.savedAt || Date.now()).toLocaleString('id-ID'))} · ${escapeHtml(entry.source || 'backup')}
                <div class="bmc-backup-sub">Data historis: ${getHistoryFootprint(entry.data)} · White: ${formatRP(balances.whiteMoney || 0)} · Black: ${formatUSD(balances.blackMoney || 0)} · Red: ${formatUSD(balances.redMoney || 0)}</div>
              </div>
              <div class="bmc-recovery-actions">
                <button class="bmc-recovery-btn" type="button" data-download="${index}">DOWNLOAD JSON</button>
                <button class="bmc-recovery-btn primary" type="button" data-restore="${index}">PULIHKAN</button>
              </div>`;
            list.appendChild(row);
        });
    }
    overlay.querySelector('#bmc-recovery-close').onclick = () => overlay.remove();
    overlay.addEventListener('click', event => {
        if (event.target === overlay) overlay.remove();
        const downloadIndex = event.target?.dataset?.download;
        if (downloadIndex !== undefined) downloadBackupJson(backups[Number(downloadIndex)]);
        const restoreIndex = event.target?.dataset?.restore;
        if (restoreIndex !== undefined) window.restoreBmcLocalBackup(Number(restoreIndex));
    });
};

function showRecoveryBanner(cloudData) {
    const best = getBestLocalBackup();
    if (!best) return;
    const cloudFootprint = getHistoryFootprint(cloudData);
    const backupFootprint = getHistoryFootprint(best.data);
    if (backupFootprint < Math.max(8, cloudFootprint + 5) || backupFootprint < cloudFootprint * 1.35) return;

    ensureRecoveryUiStyles();
    let banner = document.getElementById('bmc-recovery-banner');
    if (!banner) {
        banner = document.createElement('div');
        banner.id = 'bmc-recovery-banner';
        banner.className = 'bmc-recovery-banner';
        document.body.appendChild(banner);
    }
    banner.innerHTML = `
      <div class="bmc-recovery-copy">
        <div class="bmc-recovery-title">BACKUP LOKAL LEBIH LENGKAP DITEMUKAN</div>
        <div class="bmc-recovery-text">Cloud saat ini berisi ${cloudFootprint} unit riwayat, sedangkan backup browser berisi ${backupFootprint}. Jangan input data baru sebelum diperiksa.</div>
      </div>
      <div class="bmc-recovery-actions">
        <button class="bmc-recovery-btn" type="button" id="bmc-recovery-dismiss">NANTI</button>
        <button class="bmc-recovery-btn primary" type="button" id="bmc-recovery-open">PERIKSA BACKUP</button>
      </div>`;
    banner.querySelector('#bmc-recovery-dismiss').onclick = () => banner.remove();
    banner.querySelector('#bmc-recovery-open').onclick = () => window.showBmcRecoveryModal();
}

window.restoreBmcLocalBackup = async function(index = 0) {
    if (!window.isAdminLoggedIn) {
        window.showBmcToast('Login sebagai PENGURUS terlebih dahulu untuk memulihkan backup.', 'warning', 5000);
        return false;
    }
    if (!hasLoadedExistingCloudDocument || !isInitialLoadComplete) {
        window.showBmcToast('Data cloud belum siap. Tunggu status CONNECTED.', 'warning', 5000);
        return false;
    }
    ...
    if (!selected?.data) {
        window.showBmcToast('Backup lokal tidak ditemukan.', 'error', 5000);
        return false;
    }
    ...
    try {
        await runTransaction(db, async transaction => {
            ...
            if (!snap.exists()) throw Object.assign(new Error('Dokumen utama Firestore tidak ditemukan.'), { code: 'bmc/doc-missing' });
            const remote = snap.data();
            saveLocalSafetyBackup('before-recovery-restore', remote);
            const nextRevision = (Number(remote._revision) || 0) + 1;
            const nextUpdated = new Date().toISOString();
            transaction.update(docRef, {
                ...cloneJson(selected.data),
                _revision: nextRevision,
                lastUpdated: nextUpdated,
                recoveredAt: nextUpdated
            });
        });
        document.getElementById('bmc-recovery-modal')?.remove();
        document.getElementById('bmc-recovery-banner')?.remove();
        window.showBmcToast('Backup lokal berhasil dipulihkan ke Firebase.', 'success', 6000);
        return true;
    } catch (error) {
        console.error('❌ Restore backup gagal:', error);
        window.showBmcToast(`Restore gagal: ${error?.message || 'Firebase menolak penyimpanan.'}`, 'error', 7000);
        return false;
    }
};

window.getBmcBackupSummary = function() {
    return getAllLocalBackups().map((entry, index) => ({
        index,
        savedAt: entry.savedAt,
        source: entry.source,
        footprint: getHistoryFootprint(entry.data)
    }));
};

// SYNC FIREBASE - CLOUD FIRST, TRANSACTION SAFE
async function saveDataToCloud() {
    if (!isInitialLoadComplete || !hasLoadedExistingCloudDocument) {
        console.warn('⚠️ Simpan diblokir: data server belum selesai dimuat / dokumen utama tidak ada.');
        window.showBmcToast('Penyimpanan diblokir sampai data Firebase selesai dimuat.', 'warning', 4500);
        return false;
    }

    const payload = buildCloudPayload();
    saveLocalSafetyBackup('before-cloud-write', payload);
    const expectedRevision = Number(lastLoadedCloudRevision) || 0;
    const expectedUpdated = lastLoadedCloudUpdated || null;

    console.log('⏳ Menulis data ke Firestore dengan proteksi konflik...');
    try {
        const commitMeta = await runTransaction(db, async transaction => {
            const snap = await transaction.get(docRef);
            if (!snap.exists()) {
                throw Object.assign(new Error('Dokumen bmc_system/brangkas_data tidak ditemukan. Penulisan diblokir agar data default tidak membuat database baru.'), { code: 'bmc/doc-missing' });
            }

            const remote = snap.data();
            const remoteRevision = Number(remote._revision) || 0;
            const remoteUpdated = remote.lastUpdated || null;

            if (remoteRevision !== expectedRevision || remoteUpdated !== expectedUpdated) {
                throw Object.assign(new Error('Data Firebase berubah dari perangkat lain. Muat data terbaru lalu ulangi tindakan.'), { code: 'bmc/stale-write' });
            }

            const remoteFootprint = getHistoryFootprint(remote);
            const localFootprint = getHistoryFootprint(payload);
            if (remoteFootprint >= 15 && localFootprint < Math.floor(remoteFootprint * 0.55)) {
                throw Object.assign(new Error(`Penyimpanan diblokir karena data lokal (${localFootprint}) jauh lebih sedikit daripada data cloud (${remoteFootprint}).`), { code: 'bmc/shrink-blocked' });
            }
            ...
            return { revision: nextRevision, updated: nextUpdated };
        });

        lastLoadedCloudRevision = commitMeta.revision;
        lastLoadedCloudUpdated = commitMeta.updated;
        lastLoadedServerData = cloneJson({ ...payload, _revision: commitMeta.revision, lastUpdated: commitMeta.updated });
        saveLocalSafetyBackup('after-cloud-write', payload);
        console.log('✅ Update data ke Firestore BERHASIL dengan proteksi anti-overwrite.');
        return true;
    } catch (error) {
        console.error('❌ Gagal simpan ke Firestore:', error);
        const code = String(error?.code || '');
        if (code.includes('stale-write')) {
            window.showBmcToast('Data baru masuk dari perangkat lain. Perubahanmu dibatalkan agar data tidak tertimpa. Tunggu sinkronisasi lalu ulangi.', 'warning', 7000);
        } else if (code.includes('shrink-blocked')) {
            window.showBmcToast('Proteksi anti-hilang aktif: penyimpanan diblokir karena data lokal terlihat jauh lebih sedikit dari data Firebase.', 'error', 8000);
        } else if (code.includes('doc-missing')) {
            window.showBmcToast('Dokumen utama Firebase tidak ditemukan. Website tidak akan membuat database kosong otomatis.', 'error', 8000);
        } else if (code.includes('permission-denied')) {
            window.showBmcToast('Firebase menolak write (permission-denied). Periksa Firestore Rules.', 'error', 7000);
        } else {
            window.showBmcToast(`Data gagal disimpan: ${error?.message || 'Periksa koneksi / Firestore Rules.'}`, 'error', 7000);
        }

        // Semua caller otomatis kembali ke snapshot server terakhir jika write gagal.
        if (lastLoadedServerData) {
            applyCloudData(cloneJson(lastLoadedServerData));
...
function initRealtimeSync() {
    ...
    onSnapshot(docRef, { includeMetadataChanges: true }, (docSnap) => {
        const fromCache = Boolean(docSnap.metadata?.fromCache);
        if (statusEl) {
            statusEl.innerText = fromCache ? '↻ Syncing' : '⚡ Connected';
            statusEl.style.backgroundColor = fromCache ? '#d97706' : '#10b981';
        }

        // Jangan anggap cache sebagai load final. Tunggu snapshot server agar write tidak memakai data stale.
        if (fromCache) {
            if (docSnap.exists() && !isInitialLoadComplete) {
        ...
        if (!docSnap.exists()) {
            console.error('❌ Dokumen bmc_system/brangkas_data tidak ditemukan. AUTO-CREATE DINONAKTIFKAN.');
            hasLoadedExistingCloudDocument = false;
            isInitialLoadComplete = false;
            if (statusEl) {
                statusEl.innerText = '⚠ Data Missing';
                statusEl.style.backgroundColor = '#b91c1c';
            }
            window.showBmcToast('Dokumen utama Firebase tidak ditemukan. Tidak ada data default yang akan ditulis otomatis.', 'error', 9000);
            const best = getBestLocalBackup();
            if (best) showRecoveryBanner({});
            return;
        }

        const data = docSnap.data();
        lastLoadedServerData = cloneJson(data);
        hasLoadedExistingCloudDocument = true;
        lastLoadedCloudRevision = Number(data._revision) || 0;
        lastLoadedCloudUpdated = data.lastUpdated || null;

        applyCloudData(data);
        // Simpan snapshot server ke ring backup SEBELUM normalisasi/migrasi lokal lebih lanjut.
        saveLocalSafetyBackup('cloud-snapshot-loaded', data);

        // Migrasi lama hanya dilakukan di memori. Tidak ada auto-write saat page load.
        migrateLegacyOrderTransactions();
        migrateLegacyStockTransactions();
        ...
        window.showBmcToast(`Koneksi Firebase bermasalah: ${error?.message || 'unknown error'}`, 'error', 6500);
    });
}
