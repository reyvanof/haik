...
// PIN admin disimpan sebagai hash PBKDF2-SHA256, bukan teks biasa.
// Versi keamanan dinaikkan agar PIN lama di Firestore otomatis diganti ke konfigurasi terbaru.
const DEFAULT_ADMIN_SECURITY = Object.freeze({
    version: 2,
    algorithm: 'PBKDF2-SHA256',
    pinSalt: 'Qk1DX0RFRkFVTFRfUElOX1Yx',
    pinHash: 'dbLuhCHP73+0DQxl5CZT1tmjC4IQsI4GkgX3bSfJWGI=',
...
function normalizeAdminSecurity(value) {
    ...
    const shouldUpgradeToDefault = sourceVersion < DEFAULT_ADMIN_SECURITY.version;
    const selected = shouldUpgradeToDefault ? DEFAULT_ADMIN_SECURITY : source;

    return {
        version: DEFAULT_ADMIN_SECURITY.version,
        algorithm: 'PBKDF2-SHA256',
        pinSalt: String(selected.pinSalt || DEFAULT_ADMIN_SECURITY.pinSalt),
        pinHash: String(selected.pinHash || DEFAULT_ADMIN_SECURITY.pinHash),
        pinIterations: Math.max(100000, Number(selected.pinIterations) || DEFAULT_ADMIN_SECURITY.pinIterations),
        recoverySalt: String(selected.recoverySalt || ''),
        recoveryHash: String(selected.recoveryHash || ''),
        recoveryIterations: Math.max(100000, Number(selected.recoveryIterations) || DEFAULT_ADMIN_SECURITY.recoveryIterations),
...
async function saveAdminSecurityOnly() {
    try {
        await setDoc(docRef, {
            adminSecurity: normalizeAdminSecurity(window.adminSecurity),
            lastUpdated: new Date().toISOString()
        }, { merge: true });
        console.log('✅ PIN admin diperbarui tanpa menimpa data lain.');
    } catch (error) {
        console.error('❌ Gagal memperbarui PIN admin:', error);
    }
}
...
function initRealtimeSync() {
    ...
    onSnapshot(docRef, (docSnap) => {
        ...
        if (docSnap.exists()) {
            ...
            window._adminSecurityUpgradeNeeded = !data.adminSecurity ||
                (Math.max(0, Number(data.adminSecurity.version) || 0) < DEFAULT_ADMIN_SECURITY.version);
        ...
        const adminSecurityUpgradeNeeded = Boolean(window._adminSecurityUpgradeNeeded);
        const vaultMigrationNeeded = Boolean(window._vaultMigrationNeeded);
        window._adminSecurityUpgradeNeeded = false;
        ...
        if (adminSecurityInitialized || adminSecurityUpgradeNeeded) {
            saveAdminSecurityOnly();
        }
    }, (error) => {
