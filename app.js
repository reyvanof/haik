...
// GLOBAL STATE
// PIN tidak lagi disimpan dalam bentuk teks biasa.
// Hash awal kompatibel dengan PIN lama, tetapi PIN 4 digit tetap mudah ditebak.
// Setelah login, segera gunakan fitur Ganti PIN dan buat recovery code.
const DEFAULT_ADMIN_SECURITY = Object.freeze({
    version: 1,
    algorithm: 'PBKDF2-SHA256',
    pinSalt: 'Qk1DX0RFRkFVTFRfUElOX1Yx',
    pinHash: 'fBLDpBDUO7jDCCnbQyk/S3afXOUcUQcJ7JvrcBF4AGM=',
...
window.adminSecurity = { ...DEFAULT_ADMIN_SECURITY };
window.isAdminLoggedIn = false;
let isInitialLoadComplete = false;
let adminPinFailedAttempts = 0;
...
function normalizeAdminSecurity(value) {
    const source = value && typeof value === 'object' ? value : {};
    const pinSalt = String(source.pinSalt || DEFAULT_ADMIN_SECURITY.pinSalt);
    const pinHash = String(source.pinHash || DEFAULT_ADMIN_SECURITY.pinHash);
    ...
    return {
        ...
        pinSalt,
        pinHash,
        pinIterations: Math.max(100000, Number(source.pinIterations) || DEFAULT_ADMIN_SECURITY.pinIterations),
        recoverySalt: String(source.recoverySalt || ''),
        recoveryHash: String(source.recoveryHash || ''),
        recoveryIterations: Math.max(100000, Number(source.recoveryIterations) || DEFAULT_ADMIN_SECURITY.recoveryIterations),
        updatedAt: source.updatedAt || null
    };
}

async function verifyAdminPin(pin) {
    const security = normalizeAdminSecurity(window.adminSecurity);
    const candidateHash = await deriveSecretHash(pin, security.pinSalt, security.pinIterations);
    return safeStringEqual(candidateHash, security.pinHash);
}

async function verifyRecoveryCode(code) {
    const security = normalizeAdminSecurity(window.adminSecurity);
    if (!security.recoveryHash || !security.recoverySalt) return false;
    const candidateHash = await deriveSecretHash(code, security.recoverySalt, security.recoveryIterations);
    return safeStringEqual(candidateHash, security.recoveryHash);
}

async function createCredentialHash(secret, iterations = 210000) {
...
function recordStockFlow({ direction, source, items, amount = 0, payType = '-', notes = '-', vaultType = VAULT_BMC, createdAt = null, time = null }) {
...
window.submitAdminPin = async function() {
    const now = Date.now();
    if (now < adminPinLockedUntil) {
        const remaining = Math.ceil((adminPinLockedUntil - now) / 1000);
    ...
    if (!pin) return alert('Masukkan PIN Pengurus.');

    const submitButton = document.getElementById('btn-submit-admin-pin');
    if (submitButton) {
    ...
    try {
        ...
        if (!valid) {
            ...
            if (adminPinFailedAttempts >= 5) {
                ...
                alert('❌ PIN salah. Login dikunci selama 60 detik.');
            } else {
                alert(`❌ PIN Pengurus salah! Percobaan ${adminPinFailedAttempts}/5.`);
            }
            return;
        }
        ...
        if (!window.adminSecurity?.recoveryHash) {
            setTimeout(() => {
                alert('⚠️ Recovery code belum dibuat. Gunakan tombol Ganti PIN untuk membuat recovery code agar fitur Lupa PIN dapat digunakan.');
            }, 250);
        }
    } catch (error) {
        console.error('Gagal memverifikasi PIN:', error);
        alert('❌ PIN tidak dapat diverifikasi. Pastikan website dibuka melalui HTTPS.');
    } finally {
        if (submitButton) {
...
window.openChangePinModal = function() {
    if (!window.isAdminLoggedIn) return alert('❌ Login sebagai Pengurus terlebih dahulu.');
    ...
    document.getElementById('change-recovery-code').value = '';
    document.getElementById('modal-change-pin').classList.add('active');
};

window.changeAdminPin = async function(e) {
    if (e) e.preventDefault();
    if (!window.isAdminLoggedIn) return alert('❌ Login sebagai Pengurus terlebih dahulu.');

    const currentPin = String(document.getElementById('change-current-pin').value || '').trim();
    const newPin = String(document.getElementById('change-new-pin').value || '').trim();
    const confirmPin = String(document.getElementById('change-confirm-pin').value || '').trim();
    const recoveryCode = String(document.getElementById('change-recovery-code').value || '').trim();

    if (!validateNewPin(newPin)) return alert('PIN baru harus terdiri dari 4–12 digit. Disarankan minimal 6 digit.');
    if (newPin !== confirmPin) return alert('Konfirmasi PIN baru tidak sama.');
    if (!window.adminSecurity?.recoveryHash && recoveryCode.length < 8) {
        return alert('Buat recovery code minimal 8 karakter agar PIN dapat dipulihkan jika lupa.');
    }
    if (recoveryCode && recoveryCode.length < 8) return alert('Recovery code minimal 8 karakter.');

    const button = document.getElementById('btn-save-change-pin');
    if (button) {
    ...
    try {
        if (!(await verifyAdminPin(currentPin))) return alert('❌ PIN lama salah.');

        const newPinCredential = await createCredentialHash(newPin);
        const nextSecurity = {
            ...normalizeAdminSecurity(window.adminSecurity),
            pinSalt: newPinCredential.salt,
            pinHash: newPinCredential.hash,
            pinIterations: newPinCredential.iterations,
            updatedAt: new Date().toISOString()
        };

        if (recoveryCode) {
            const recoveryCredential = await createCredentialHash(recoveryCode);
            nextSecurity.recoverySalt = recoveryCredential.salt;
            nextSecurity.recoveryHash = recoveryCredential.hash;
            nextSecurity.recoveryIterations = recoveryCredential.iterations;
        }

        window.adminSecurity = nextSecurity;
        await window.saveData();
        window.closeModal('modal-change-pin');
        alert('✅ PIN Pengurus berhasil diganti. Simpan recovery code di tempat aman.');
    } catch (error) {
        console.error('Gagal mengganti PIN:', error);
        alert('❌ PIN gagal diganti. Periksa koneksi dan izin Firestore.');
    } finally {
        if (button) {
            button.disabled = false;
            button.textContent = 'Simpan PIN Baru';
        }
    }
};
...
window.openForgotPinModal = function() {
    ...
    document.getElementById('modal-pin').classList.remove('active');
    document.getElementById('modal-forgot-pin').classList.add('active');
};

window.resetAdminPinWithRecovery = async function(e) {
    if (e) e.preventDefault();

    if (!window.adminSecurity?.recoveryHash) {
        return alert('❌ Recovery code belum pernah dibuat. PIN tidak dapat direset dari browser.');
    }

    const recoveryCode = String(document.getElementById('forgot-recovery-code').value || '').trim();
    const newPin = String(document.getElementById('forgot-new-pin').value || '').trim();
    const confirmPin = String(document.getElementById('forgot-confirm-pin').value || '').trim();

    if (!recoveryCode) return alert('Masukkan recovery code.');
    if (!validateNewPin(newPin)) return alert('PIN baru harus terdiri dari 4–12 digit. Disarankan minimal 6 digit.');
    if (newPin !== confirmPin) return alert('Konfirmasi PIN baru tidak sama.');

    const button = document.getElementById('btn-reset-pin');
    if (button) {
        button.disabled = true;
    ...
    try {
        if (!(await verifyRecoveryCode(recoveryCode))) return alert('❌ Recovery code salah.');

        const credential = await createCredentialHash(newPin);
        ...
        await window.saveData();
        window.closeModal('modal-forgot-pin');
        document.getElementById('user-role').value = 'member';
        alert('✅ PIN berhasil direset. Silakan pilih Pengurus dan login dengan PIN baru.');
    } catch (error) {
        console.error('Gagal reset PIN:', error);
        alert('❌ PIN gagal direset. Periksa koneksi dan izin Firestore.');
    } finally {
        if (button) {
            button.disabled = false;
...
window.renderBrangkas = function() {
    ...
    const renderVaultRows = (tbodyId, vaultType) => {
        ...
        tbody.innerHTML = entries.map(([itemName, qty]) => {
...
${details}`);
    ...
    window.cartItems.forEach(item => {
        ...
        vaultItems[stockItemName] = (Number(vaultItems[stockItemName]) || 0) - item.qty;
    });
    ...
    renderAll();
    alert(`✅ Penjualan berhasil disimpan dari ${getVaultLabel(vaultType)}!`);
};
...
window.processTransaction = async function(e) {
    ...
    const payType = document.getElementById('modal-pay-type').value;
    const vaultType = normalizeVaultType(document.getElementById('modal-vault-type')?.value || VAULT_BMC);
    const priceUP = parseFloat(document.getElementById('modal-price-up').value) || 0;
    const priceBM = parseFloat(document.getElementById('modal-price-bm').value) || 0;
    ...
    const vaultItems = getVaultItems(vaultType);
    const currentStock = Number(vaultItems[itemName]) || 0;

    if (isBmcToKelompok && currentStock < qty) {
        return alert(`❌ Stok ${itemName} di ${getVaultLabel(vaultType)} hanya ${currentStock} PCS, sedangkan order ${qty} PCS.`);
    }
    ...
    if (isBmcToKelompok) {
        ...
        vaultItems[itemName] = currentStock - qty;
    } else {
        if (payType === 'UP') {
    ...
    alert(`✅ Order berhasil dicatat melalui ${getVaultLabel(vaultType)}!`);
};
...
window.renderLowStock = function() {
    ...
    const lowStock = [VAULT_BMC, VAULT_BOSS].flatMap(vaultType =>
        Object.entries(getVaultItems(vaultType)).map(([name, qty]) => ({
        ...
        .sort((a, b) => a.qty - b.qty || a.vaultType.localeCompare(b.vaultType) || a.name.localeCompare(b.name));
    ...
    container.innerHTML = lowStock.map(item => `
        <div class="low-stock-item">
            <div>
                ...
                <div class="low-stock-category">${escapeHtml(item.category)} • ${getVaultLabel(item.vaultType)}</div>
            </div>
...
window.renderSalesData = function() {
    ...
    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" class="dashboard-empty">Belum ada data penjualan.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = records.map(({ record, sourceIndex }) => {
        ...
        const vaultItems = getVaultItems(vaultType);
        const remainingStockText = (record.items || []).length
            ? (record.items || []).map(item => {
                const currentQty = Number(vaultItems[normalizeItemName(item.name)]) || 0;
                return `${item.name}: ${currentQty} PCS`;
            }).join(', ')
            : '-';
        return `
            <tr>
                ...
                <td class="stock-activity-notes">${escapeHtml(remainingStockText)}</td>
...
window.saveSaleEdit = async function(e) {
    ...
    // Kembalikan stok dari penjualan lama ke brangkas asal.
    ...
    if (shortages.length > 0) {
        const details = shortages.map(item => {
            ...
            return `${item.name}: tersedia ${available} PCS, dibutuhkan ${item.qty} PCS`;
        }).join('\n');
...
${details}`);
    ...
    window.stockFlowData[sourceIndex] = {
        ...
        items: newItems,
        totalQty: newItems.reduce((sum, item) => sum + item.qty, 0),
        amount: newAmount,
        payType: newPayType,
        notes: `Pembeli: ${buyer} | Penjual: ${seller}`,
        updatedAt: new Date().toISOString()
    ...
    alert('✅ Data penjualan, tipe brangkas, stok, dan saldo berhasil diperbarui.');
};
...
function buildStockMovementRows(direction) {
    ...
    if (records.length === 0) {
        ...
        return `<tr><td colspan="8" class="dashboard-empty">Belum ada ${label}.</td></tr>`;
    }
    ...
    return records.map(({ record, sourceIndex }) => {
        ...
        const stockLines = (record.items || []).map(item => {
            const current = Number(vaultItems[normalizeItemName(item.name)]) || 0;
...
async function saveDataToCloud() {
    ...
    try {
        await setDoc(docRef, {
            ...
            adminSecurity: normalizeAdminSecurity(window.adminSecurity),
...
function initRealtimeSync() {
    ...
    onSnapshot(docRef, (docSnap) => {
        ...
        if (docSnap.exists()) {
            ...
            window.adminSecurity = normalizeAdminSecurity(data.adminSecurity);
            normalizeOrderHistoryData();
            normalizeLoanRecordsData();
        } else {
            ...
            window.stockFlowData = [];
            ensureVaultState();
            window.adminSecurity = normalizeAdminSecurity(null);
            normalizeOrderHistoryData();
            normalizeLoanRecordsData();
        ...
        const adminSecurityInitialized = docSnap.exists() && !docSnap.data().adminSecurity;
        const vaultMigrationNeeded = Boolean(window._vaultMigrationNeeded);
        window._vaultMigrationNeeded = false;
        isInitialLoadComplete = true;
        renderAll();

        if (legacyOrdersMoved || legacyStockMoved || adminSecurityInitialized || vaultMigrationNeeded) {
            saveDataToCloud();
        }
    }, (error) => {
