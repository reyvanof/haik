...
window.checkoutMemberCart = async function() {
    ...
    const vaultType = normalizeVaultType(document.getElementById('cart-vault-type')?.value || VAULT_BMC);
...
window.renderSalesData = function() {
    ...
    if (records.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" class="dashboard-empty">Belum ada data penjualan.</td></tr>';
        return;
    }
