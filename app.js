...
window.checkoutMemberCart = async function() {
    ...
    try {
        ...
        alert('✅ Order berhasil dikirim dan masuk ke Data Penjualan Pengurus dengan status PENDING.');
    } catch (error) {
...
window.processMemberPendingOrder = async function(orderId) {
    if (!window.isAdminLoggedIn || !auth.currentUser || !currentAdminProfile) {
        return alert('❌ Login Pengurus diperlukan.');
    }
    ...
    if (!confirm(`Proses order dari ${pendingOrder.buyer || '-'}?`)) return;
    ...
    try {
        ...
        await writeAuditLog('PROCESS_MEMBER_SALE', { orderId });
        ...
        .forEach(order => {
            ...
            newRows.push(`
                <tr>
                    ...
                    <td><span style="color:var(--text-muted);">Belum diproses</span></td>
                    <td>
                        <div class="sales-action-buttons">
                            <button class="btn btn-sm btn-green" type="button"
                                onclick="window.processMemberPendingOrder('${order.id}')">Proses</button>
                            <button class="btn btn-sm btn-red" type="button"
                                onclick="window.rejectMemberPendingOrder('${order.id}')">Tolak</button>
                        </div>
                    </td>
                </tr>`);
        });
