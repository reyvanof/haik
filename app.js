...
window.openManualOrderProcess = function(type) {
    ...
    document.getElementById('modal-pay-type').value = 'UP';
    document.getElementById('modal-vault-type').value = isBmcToKelompok ? 'BMC' : 'BOSS';
...
window.calcModalTotal = function() {
    ...
    document.getElementById('modal-total-price').value = payType === 'UP'
        ? formatRP(total)
        : formatUSD(total);
};
...
window.processTransaction = async function(e) {
    ...
    // Order manual hanya dicatat ke Riwayat Order. Tidak mengecek, mengurangi,
    // atau menambah stok brangkas, sehingga proses tidak bergantung pada ketersediaan barang.
    window.orderHistoryData = [orderRecord, ...window.orderHistoryData];

    await window.saveData();
    renderAll();
    window.closeModal('modal-sell');
    alert(`✅ Order manual berhasil dicatat. Total: ${orderRecord.total}`);
};

window.openCustomOrderModal = function(target) {
    document.getElementById('custom-target-panel').value = target;
    document.getElementById('modal-custom-title').innerText = `Input Harga Custom: ${target}`;
...
window.saveCustomOrder = async function(e) {
    ...
    alert('✅ Harga Custom Berhasil Ditambahkan!');
};
    ...
    tbody.innerHTML = records.map(({ record, sourceIndex }) => {
        ...
        const remainingStockHtml = (record.items || []).length
            ? (record.items || []).map(item => {
                ...
                const badgeClass = currentQty <= 0 ? 'badge-red' : 'badge-green';
                return `<div style="margin-bottom:4px;"><span class="badge ${badgeClass}">${escapeHtml(item.name)}: ${currentQty} PCS</span></div>`;
            }).join('')
            : '-';
